...
__global__ void redUpdate(float *C_gpu, float *e_gpu, int nX, int nY) {
	...
	for (index_x = start_x; index_x < end_x; index_x ++ ) {
		for (index_y = start_y + ((index_x + start_y) % 2); index_y < end_y; index_y += 2) {
			...
			//update local delta
			if (abs(*(C_gpu + index_x * N + index_y) - (u + d + l + r)/4) > e) {
				e = abs(*(C_gpu + index_x * N + index_y) - (u + d + l + r)/4);
			}
	...
	*(e_gpu + (blockIdx.x*blockDim.x + threadIdx.x)*blockDim.y*BlocksY + blockIdx.y*blockDim.y + threadIdx.y) = e;
}
...
__global__ void blackUpdate(float *C_gpu, float *e_gpu, int nX, int nY) {
	...
	if (*(e_gpu + (blockIdx.x*blockDim.x + threadIdx.x)*blockDim.y + threadIdx.y) > e) {
	  *(e_gpu + (blockIdx.x*blockDim.x + threadIdx.x)*blockDim.y*BlocksY + blockIdx.y*blockDim.y + threadIdx.y) = e;
	}
}
...
__global__ void rowWiseReduction(float *e_gpu, int stride) {
  ...
  if (index_y % stride == 0) {
    if (*(e_gpu + index_x*blockDim.y*BlocksY + index_y) < *(e_gpu + index_x*blockDim.y*BlocksY + index_y + stride/2)) {
      *(e_gpu + index_x*blockDim.y*BlocksY + index_y) = *(e_gpu + index_x*blockDim.y*BlocksY + index_y + stride/2);
    }
  }
}
...
__global__ void columnWiseReduction(float *e_gpu, int stride) {
  ...
  if ((index_x % stride == 0) && (index_y == 0)) {
    if (*(e_gpu + index_x*blockDim.y*BlocksY + index_y) < *(e_gpu + (index_x + stride / 2)*blockDim.y*BlocksY + index_y)) {
      *(e_gpu + index_x*blockDim.y*BlocksY + index_y) = *(e_gpu + (index_x + stride / 2)*blockDim.y*BlocksY + index_y);
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  /*
	stopping variable e
  e_gpu is 2D array, stores local maximum changing of threads
  e_gpu(i, j) is local maximum of thread (i, j)
  */
	float *e_gpu, e = 1;
	cudaMalloc((void**) &e_gpu, (BlocksX*BlocksY*ThreadsX*ThreadsY)*sizeof(float));
  ...
  iter_name = (char *)malloc(20*sizeof(char));
    ...
    //update red point in gpu
		redUpdate<<<dimGrid, dimBlock>>>(C_gpu, e_gpu, nX, nY);

    //update black points in gpu
		blackUpdate<<<dimGrid, dimBlock>>>(C_gpu, e_gpu, nX, nY);
    ...
    */
    stride = 2;
    do {
      rowWiseReduction<<<dimGrid, dimBlock>>>(e_gpu, stride);
      stride *= 2;
    ...
    */
    stride = 2;
    do {
      columnWiseReduction<<<dimGrid, dimBlock>>>(e_gpu, stride);
      stride *= 2;
    } while (stride <= BlocksY*ThreadsY);

    //send max changing to CPU
    cudaMemcpy(&e, e_gpu, sizeof(float), cudaMemcpyDeviceToHost);
